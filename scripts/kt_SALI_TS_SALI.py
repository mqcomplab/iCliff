...
for file in glob.glob('data/props/*.npy'):
    ...
    # Calculate the abs diff property matrix
    abs_diff = abs_pair_prop_diff_matrix(props)
    ...
    for fp in ['RDKIT', 'ECFP4', 'MACCS']:
        ...
        sali = sali_matrix(abs_diff, sim)

        for term in [1, 2, 3]:
            # Calculate the Taylor sum SALI matrix
            ts_sali = ts_sali_matrix(squared_diff, sim, term)

            # Save the matrix for term 3
            if term == 3 and fp == 'ECFP4':
                np.save(f'data/ts_sali_matrix/{fp}/{name}.npy', ts_sali)
...
df.to_csv('results/kt_SALI_TS_SALI.csv', index = False)
